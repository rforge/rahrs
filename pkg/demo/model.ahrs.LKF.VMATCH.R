...
Parameters<-list(mn=0,an=0,lambda=0)
...
#Initial Quaternion
q = matrix(c( 1.0,0.0,0.0,0.0 ) ,nrow=1,ncol=4)

#Initial Covariance matrix
Filter$P = diag(c(1e1, 1e1, 1e1))
#System noises
Filter$Q = diag(c(1e-2, 1e-2, 1e-2))
#Measurement noises
...
for (n in 1:Nsim)
    ...
    ## EKF Part
tmp <- ahrs_LKF_VMATCH(Filter, Sensors, q, Parameters)
Filter <- tmp$Filter
q <- tmp$q    

    #Calculate AHTS angles from estimated quaternion Q
    R_[n,] = Q2EA( Qconj(q) )
...
list(A_=A_, a_=a_, m_=m_, R_=R_, TRIAD_=TRIAD_)
}
...
plot(TRIAD_[,1],col='red',ylim=c(-4,4), type='l',main='LKF VMATCH TRIAD / AHRS',xlab='Time 10 msec (100 Hz)', ylab='Euler angles')
par(new=T)
